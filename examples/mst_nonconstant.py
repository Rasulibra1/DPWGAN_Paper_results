# ============================================================
# Higher-order interaction benchmark (DPWGAN) — NONCONSTANT ONLY
# (rho12/rho13/rho23), includes a sweep over beta_int values
#
# For each beta_int:
#   - simulate once (with SIM_SEED_BASE + offset)
#   - split once (with SPLIT_SEED_BASE + offset)
...
#
# Notes:
# - Uses the repo's DPWGAN training loop (gradient noise via sigma + weight clipping).
# - Treats V1..V4 as continuous; we standardize using TRAIN stats for stable GAN training.
# - "epsilon" here is used as an index into a user-provided mapping EPS -> SIGMA / EPOCHS.
#   This matches the style of your earlier dpwgan runner (epochs-by-eps).
# ============================================================
...
# Import the DPWGAN class from your repo.
...
# -------------------------------
# 0) Simulate benchmark data (NONCONSTANT)
# -------------------------------
...
# -------------------------------
# 7) Main experiment with beta_int sweep (NONCONSTANT)
# -------------------------------
def main():
    logging.basicConfig(level=logging.INFO)

    SIM_SETUP = "nonconstant"
    eps_grid = [1, 2, 3, 4, 5]
    reps = list(range(1, 6))
    beta_int_grid = [0, 0.5, 1, 2, 3, 5]

    # Nonconstant sim params
    N_SIM_NC = 20_000
    ...
    # DPWGAN hyperparams (tune freely)
    ...
    # IMPORTANT:
    # This repo's DPWGAN takes sigma, not epsilon. So we need a mapping.
    # Use YOUR mapping here. Example below is placeholder.
    # If you already use epochs-by-eps, keep that style and choose sigma fixed.
    ...
    for b_idx, beta_int in enumerate(beta_int_grid, start=1):
        ...
        for eps in eps_grid:
            ...
            for r in reps:
                ...
                synth_path = os.path.join(out_dir_beta, f"eps_{eps_int}", f"synthetic_rep_{int(r):02d}.csv")
    ...
    out_main = os.path.join(OUT_ROOT, f"higher_order_dpwgan_{SIM_SETUP}_betaIntSweep_results.csv")
    ...
    out_sum = os.path.join(OUT_ROOT, f"higher_order_dpwgan_{SIM_SETUP}_betaIntSweep_summary.csv")
